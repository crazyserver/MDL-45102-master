...
class core_message_renderer extends plugin_renderer_base {
    ...
    public function manage_defaultmessageoutputs($processors, $providers, $preferences) {
        ...
        foreach ($components as $component) {
            $component_export = new StdClass();
            $component_export->name = $component;

            if ($component != 'moodle') {
                $component_export->displayname = get_string('pluginname', $component);
            } else {
                $component_export->displayname = get_string('coresystem');
            }

            $component_export->providers = array();
            foreach ($providers as $provider) {
                if ($provider->component == $component) {
                    $component_export->providers[] = $provider;
                }
            }
            $components_export[] = $component_export;
        }

        $context['processors'] = array_values($processors);
        $context['components'] = $components_export;
        $context['colspan'] = count($processors) + 2;
